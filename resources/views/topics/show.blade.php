    ...
    <div class="col-md-9 topics-show main-col">
        ...
        <div class="topic panel panel-default">
            <div class="infos panel-heading">
                ...
                <div class="votes">
                    ...
                    <a data-method="post" href="javascript:void(0);"
                       data-url="{{ route('topics.upvote', $topic->id) }}"
                       title="{{ lang('Up Vote') }}" id="up-vote"
                       class="vote {{ auth()->check() && $topic->votes()->ByWhom(auth()->id())->WithType('upvote')->count() ? 'active' :'' }}">
                        <li class="fa fa-chevron-up"></li> {{ $topic->vote_count }}
                    </a>
                    &nbsp;
                    <a data-method="post" href="javascript:void(0);"
                       data-url="{{ route('topics.downvote', $topic->id) }}"
                       title="{{ lang('Down Vote') }}" id="down-vote"
                       class="vote {{ auth()->check() && $topic->votes()->ByWhom(auth()->id())->WithType('downvote')->count() ? 'active' :'' }}">
            ...
            @foreach ($topic->appends as $index => $append)
                ...
                <div class="appends">
                    ...
                    <div class="sep5"></div>
                    <div class="markdown-reply append-content">{!! $append->content !!}</div>
                </div>
        ...
        <div class="replies panel panel-default list-panel replies-index">
            <div class="panel-heading">
                <div class="total">{{ lang('Total Reply Count') }}:
                    <b>{{ $replies->total() }}</b></div>
            </div>
